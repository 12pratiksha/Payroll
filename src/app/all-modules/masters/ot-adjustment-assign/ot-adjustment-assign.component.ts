...
getAll(){
  ...
  res.forEach(element => {
    ...
    if(resultBranch.length>0){
      brch=resultBranch[0].branchName
    }
    this.employee_display.push({"employeeId":element.employeeId,"employeeCode":element.employeeCode,"firstName":element.firstName,"lastName":element.lastName,"branch":brch,"department":dept,"status":element.status})
    })
