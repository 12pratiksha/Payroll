...
@Component({
  selector: 'app-import-salary-with-formula',
  templateUrl: './import-salary-with-formula.component.html',
  styleUrls: ['./import-salary-with-formula.component.css']
})
export class ImportSalaryWithFormulaComponent implements OnInit {
  @ViewChild('myInput')
...
this.importDesignaton=this._fb.group({
  ...
  exportexcel(): void
  {
    ...
    let url = this.host+"/assets/Excle/ImportSalaryWithFormula.xlsx";
    this.filenameurl=url;
    console.log(url);
    this.httpClient.get(url, { responseType: 'blob' }).subscribe(val => {
      // this.zipdownload=true;
      const url = URL.createObjectURL(val);
      this.downloadUrl(url, "importSalaryWithFormula" + '.xlsx');
  ...
  downloadUrl(url: string, fileName: string) {
    ...
    a.href = url;
    a.download = fileName;
    document.body.appendChild(a);2222
...
if (this.file.type=="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"){
 ...
 let formData = new FormData();
    ...
    this._service.uploadFile(formData, 'Salary_master/getsheets').subscribe((res)=>{
      console.log(res);
...
Swal.fire({
  ...
  didOpen: () => {
    Swal.showLoading()
     ...
     this._service.uploadFile(formData, 'Salary_master/uploadfileexcel').subscribe((res)=>{
