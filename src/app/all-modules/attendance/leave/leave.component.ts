...
submit(){
  if(this.leaveApplicationForm.valid && this.userType!='Admin' || this.userType!='Super Admin'){
...
this._service.add(data, "LeaveApplication/addLeaveApplication").subscribe((res)=>{
  ...
  else if(this.leaveApplicationForm.valid && this.userType=='Admin' || this.userType=='Super Admin' ){
    let leaves = []
  for (let i = 0; i < this.leaveApplicationForm.value.leave.length; i ++){
  var datePipe = new DatePipe('en-US');
  let fromDate = datePipe.transform(this.leaveApplicationForm.value.fArray[i].fromDate, 'yyyy-MM-dd');
...
let emp=this.leaveApplicationForm.value.employee.split(" ");
...
if(this.userType=='Admin' ){
  if(this.leaveApplicationForm.value.employee){
    console.log(this.leaveApplicationForm.value.employee)
    let emp=this.leaveApplicationForm.value.employee.split(" ");
     let emply=this.employees.filter((emply)=>
     (emply.employeeCode == emp[0]))
     console.log(emply)
     ...
     this.leaveApplicationForm.controls.reason.reset()
       ...
       let url = "getempId/"+`${emply[0].employeeId}`
      this._service.get(url).subscribe((res)=>{
       if(res.length < 1){
         Swal.fire({
           icon: 'error',
...
//   let form=this.leaveApplicationForm.value
//   if(this.array.length >=2){

//   let url='employees/search?employeeName='+`${form.employee}`
//   this._service.get(url).subscribe((res)=>{
