...
@Component({
  selector: 'app-p-fregister-report',
  templateUrl: './p-fregister-report.component.html',
  styleUrls: ['./p-fregister-report.component.css']
})
export class PFRegisterReportComponent implements OnInit {
...
console.log(this.month+"---------------"+this.year1)
  ...
  if(this.filterForm.status=='VALID'){
    if(this.filterForm.value.employeeId && branchStatus=='true' && form.branch==''){
     ...
     console.log(emply)
  let url='getPFRegisterReport?month='+form.month+'&year='+form.year+'&state='+form.state+'&status='+form.status+'&employeeNo='+emply[0].employeeId+'&category='+form.category+'&department='+form.department+'&branch='+branchId
  this.service.get(url).subscribe((res)=>{
    this.data=res
    console.log(res)
 ...
 (emply.employeeCode == emp[0]))
 console.log(emply)
let url='getPFRegisterReport?month='+form.month+'&year='+form.year+'&state='+form.state+'&status='+form.status+'&employeeNo='+emply[0].employeeId+'&category='+form.category+'&department='+form.department+'&branch='+form.branch
this.service.get(url).subscribe((res)=>{
this.data=res
console.log(res)
...
console.log(res)
  ...
  let url='getPFRegisterReport?month='+form.month+'&year='+form.year+'&state='+form.state+'&status='+form.status+'&employeeNo='+''+'&category='+form.category+'&department='+form.department+'&branch='+form.branch
  this.service.get(url).subscribe((res)=>{
    this.data=res
    console.log(res)
