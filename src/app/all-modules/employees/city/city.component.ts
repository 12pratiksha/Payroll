...
this.addcityForm = this.fb.group({
  ...
  let url = `${'state/getState/'}`+`${state}`
    this.srvModuleService.get(url).subscribe((res1)=>{
      if(res1){
         this.stateName=res1.stateName
        //  console.log(res1)
      }
      let url = 'country/getCountry/'+`${country}`
      this.srvModuleService.get(url).subscribe((res2)=>{
      if(res2){
        this.countryName=res2.countryName
      }

     let url = 'region/getRegion/'+`${region}`
     this.srvModuleService.get(url).subscribe((res3)=>{
      if(res3){
        this.regionName=res3.regionName
      }
