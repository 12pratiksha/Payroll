...
export class OfferComponent implements OnInit {
  ...
  this.baseurl=localStorage.getItem('baseurl')
    ...
    if(param){
      ...
      this.status = this.roundId.status;
      this.getDetails()
...
this.getDesignation()
  ...
  serach(){
    ...
    this._service.get(url).subscribe((res)=>{
      ...
      this.sampleContent=res.samplecontent
  ...
  submit()
  {
    ...
    let data={
      ...
      "emailSubject": "Job Offer from [Company Name] - Congratulations!",
    ...
    this.offerLetter=res.data.sendofferletterName
    let url2='AddCandiateOfferHistroy' 
    let data={
    "applicantCandiateListId": this.details[0].applicant_candiate_list_id,
    "requirementId":this.details[0].requirement_id,
    "offerSalary":form1.Offered,
    "offerlattername":res.data.sendofferletterName,
    "offerlatterLocation":res.data.location,
}
    this._service.add(data,url2).subscribe((res)=>{
      Swal.fire({
        icon: 'success',
