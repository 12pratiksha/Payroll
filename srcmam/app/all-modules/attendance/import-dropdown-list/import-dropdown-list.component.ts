...
@Component({
  selector: 'app-import-dropdown-list',
  templateUrl: './import-dropdown-list.component.html',
  styleUrls: ['./import-dropdown-list.component.css']
})
export class ImportDropdownListComponent implements OnInit {
  @ViewChild('myInput')
  ...
  exportexcel(): void
  {
    ...
    let url = this.host+"/assets/Excle/LookUpMaster.xlsx";
    this.filenameurl=url;
    console.log(url);
    this.httpClient.get(url, { responseType: 'blob' }).subscribe(val => {
   
      const url = URL.createObjectURL(val);
      this.downloadUrl(url, "LookUpMaster" + '.xlsx');
 ...
 let formData = new FormData();
    ...
    this._service.uploadFile(formData, 'LookupMaster/getsheets').subscribe((res)=>{
      this.sheet =res;
...
Swal.fire({
  ...
  didOpen: () => {
    Swal.showLoading()
     ...
     this._service.uploadFile(formData, 'LookupMaster/uploadfileexcel').subscribe((res)=>{
