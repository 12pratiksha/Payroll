...
export class ESICStatementReportComponent implements OnInit {
  ...
  console.log(this.month+"---------------"+this.year1)
    ...
    let url='getESICStatementReport?month='+form.month+'&year='+form.year+'&state='+form.state+'&status='+form.status+'&employeeNo='+form.employeeId+'&category='+form.category+'&department='+form.department+'&branch='+form.branch
    this.service.get(url).subscribe((res)=>{
      this.data=res
      console.log(res)
